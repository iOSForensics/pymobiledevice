...
class BPlistReader(object):
    ...
    def __unpackItem(self, offset):
        ...
        if obj_type == 0x00:
            if obj_info == 0x00:  # null   0000 0000
                ...
                raise Exception(
                    'unpack item type ' + str(obj_header) + ' at ' + str(offset) + 'failed')
        ...
        # 1111 then int count follows, followed by bytes
        elif obj_type == 0x40:
        ...
        # string  0101 nnnn   [int]   ... // ASCII string, nnnn is # of chars,
        # else 1111 then int count, then bytes
        elif obj_type == 0x50:
            obj_count, objref = self.__resolveIntSize(obj_info, offset)
        ...
        # chars, else 1111 then int count, then big-endian 2-byte uint16_t
            ...
            obj_count, objref = self.__resolveIntSize(obj_info, offset)
            return plistlib.Data(self.data[objref:objref + obj_count])
        # array   1010 nnnn   [int]   objref* // nnnn is count, unless '1111',
        # then int count follows
        elif obj_type == 0xA0:
            obj_count, objref = self.__resolveIntSize(obj_info, offset)
            arr = []
            for i in range(obj_count):
                arr.append(self.__unpackIntStruct(self.object_ref_size, self.data[
                           objref + i * self.object_ref_size:objref + i * self.object_ref_size + self.object_ref_size]))
            return arr
        ...
        # '1111', then int count follows
            ...
            keys = []
            for i in range(obj_count):
                keys.append(self.__unpackIntStruct(self.object_ref_size, self.data[
                            objref + i * self.object_ref_size:objref + i * self.object_ref_size + self.object_ref_size]))
            values = []
            objref += obj_count * self.object_ref_size
            for i in range(obj_count):
                values.append(self.__unpackIntStruct(self.object_ref_size, self.data[
                              objref + i * self.object_ref_size:objref + i * self.object_ref_size + self.object_ref_size]))
            dic = {}
            for i in range(obj_count):
                dic[keys[i]] = values[i]
    ...
    def parse(self):
        ...
        # print "** plist
        # offset_size:",self.offset_size,"objref_size:",self.object_ref_size,"num_objs:",self.number_of_objects,"top:",self.top_object,"table_ofs:",self.table_offset
